...
interface PromptInputBoxProps {
  ...
  placeholder?: string;
  className?: string;
  autoSubmitPdf?: boolean;
  pdfAutoSubmitMessage?: string;
}
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  const processFile = (file: File) => {
    ...
    if (isPdfFile(file) && autoSubmitPdf) {
      onSend(pdfAutoSubmitMessage, [file]);
      setFiles([]);
      setFilePreviews({});
      return;
    }
      ...
      >
            ...
            {files.map((file, index) => (
              <div key={index} className="relative group">
                ...
                {file.type === "application/pdf" && (
                  <div className="relative flex min-w-[180px] max-w-[240px] items-center gap-2 rounded-xl border border-[#444444] bg-[#2A2B2F] px-3 py-2 text-sm text-gray-100">
                    <Paperclip className="h-4 w-4 shrink-0 text-[#D1D5DB]" />
                    <span className="truncate">{file.name}</span>
                    <button onClick={() => handleRemoveFile(index)} className="ml-auto rounded-full bg-black/40 p-1">
                      <X className="h-3 w-3 text-white" />
                    </button>
                  </div>
                )}
        ...
        {isRecording && (
          <VoiceRecorder
            ...
            onStartRecording={() => {}}
            onStopRecording={(duration) => { setIsRecording(false); onSend(`[Voice message - ${duration} seconds]`, []); }}
          />
        )}
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          <div className={cn("flex items-center gap-1 transition-opacity duration-300", isRecording ? "opacity-0 invisible h-0" : "opacity-100 visible")}>
            <PromptInputAction tooltip="Upload image or PDF">
              <button onClick={() => uploadInputRef.current?.click()} className="flex h-8 w-8 text-[#9CA3AF] cursor-pointer items-center justify-center rounded-full transition-colors hover:bg-gray-600/30 hover:text-[#D1D5DB]" disabled={isRecording}>
                ...
                <input ref={uploadInputRef} type="file" className="hidden" onChange={(e) => { if (e.target.files?.[0]) processFile(e.target.files[0]); if (e.target) e.target.value = ""; }} accept="image/*,application/pdf" />
              </button>
            </PromptInputAction>
          ...
          <PromptInputAction tooltip={isLoading ? "Stop" : isRecording ? "Stop recording" : hasContent ? "Send" : "Voice message"}>
            <Button
              variant="default"
              size="icon"
              className={cn("h-8 w-8 rounded-full transition-all duration-200", isRecording ? "bg-transparent hover:bg-gray-600/30 text-red-500" : hasContent ? "bg-white hover:bg-white/80 text-[#1F2023]" : "bg-transparent hover:bg-gray-600/30 text-[#9CA3AF] hover:text-[#D1D5DB]")}
